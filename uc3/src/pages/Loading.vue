...
export default {
  ...
  methods: {
    ...
    getOrderStatus (orderId) {
      if (orderId) {
        axios.get(this.$store.state.endAPIRequest + 'nespresso/orders/' + orderId)
          .then(response => {
            if (response.data.status === 'complete') {
              this.$store.commit('storeCompletedOrder', response.data)
              router.push({ name: 'Success', params: { orderId }, query: { utm_source: this.$store.state.utm_source, utm_medium: this.$store.state.utm_medium, utm_campaign: this.$store.state.utm_campaign } })
    ...
    createOrder () {
      axios.post('/V1/nespresso/orders/create', this.$store.state.orders)
        .then(response => {
