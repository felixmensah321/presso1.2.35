...
class Order
{
     ...
     * @throws \Exception
     */
    private function getProvider($isBis)
    {
        $provider = $this->providerFactory->create();
        if ($isBis) {
            $orders = $this->orderEntity->getBisYesterdayOrders();
        } else {
            $orders = $this->orderEntity->getYesterdayOrders();
        }
    ...
    {
        try {
            $this->xmlCreator->createXmlFileFromProvider($this->getProvider(false), false);
        } catch (\Exception $e) {
     ...
     * Create order export xml file.
     */
    public function createOrderExportFileBis()
    {
        try {
            $this->xmlCreator->createXmlFileFromProvider($this->getProvider(true), true);
        } catch (\Exception $e) {
            $this->logger->debug('[SQLi][EXPORT] Exception during order export.');
            $this->logger->debug($e->getMessage());
        }
