...
export default {
  ...
  methods: {
    ...
    getSKU (sku) {
      axios.get(this.$store.state.endAPIRequest + 'products/' + sku)
        .then(response => {
    ...
    getCategoryProducts (categoryId) {
      axios.get(this.$store.state.endAPIRequest + 'products?searchCriteria[filter_groups][0][filters][0][field]=category_id&searchCriteria[filter_groups][0][filters][0][value]=' + categoryId + '&searchCriteria[filter_groups][0][filters][0][condition_type]=eq&searchCriteria[filter_groups][0][filters][1][field]=status&searchCriteria[filter_groups][0][filters][1][value]=1&searchCriteria[filter_groups][0][filters][1][condition_type]=eq&searchCriteria[sortOrders][0][field]=position&searchCriteria[sortOrders][0][direction]=DESC')
        .then(response => {
    ...
    getCategoryArchitecture (categoryId, level) {
      axios.get(this.$store.state.endAPIRequest + 'categories/' + categoryId).then(response => {
        let obj = {
          id: categoryId,
